...
public class CpeHintDao {
	...
	public int addDependencyHint(JdbcTemplate jdbcTemplate, CpeHint hint) {
	    ...
	    int count = jdbcTemplate.queryForObject(checkQuery, Integer.class, new Object[]{hint.getType(), hint.getMatch_key(), hint.getEvidenceType()});
	    ...
	    if (count > 0) {
	        ...
	        return jdbcTemplate.update(updateQuery, new Object[]{
	            hint.getStandardized_name(),
	            hint.getConfidence(),
	            hint.getDescription(),
	            hint.getEvidenceType(),
